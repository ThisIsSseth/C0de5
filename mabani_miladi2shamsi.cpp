...
int main()
{
    ...
    while (1)
    {
        ...
        cin >> day >> month >> year;
        // days = total days since مبدا تقویم میلادی
        ...
        days -= 226899;
        // 226899 is the difference of the start of the miladi and shamsi
        days -= days / (365 * 4 + 1);
        s_year = days / 365;
        days = days % 365;
        // // cout << days << endl;
        // below : days / 31 (we assume all months are 31 day and evaluate the num of months)
        // then we add one more to month if the number of true remaining days is more than 30 if we're in the second 6 months of the year
        ...
        // s_month = days / 31 + ((days / 31 / 6)  * ((days % 31 + ((days / 31) % 6)) / 30));
        // coudln't figure out the bug so imma rewrite it
        s_month = days / 31;
        int temp_days = days % 31;
        // if (month > 6)
        //     temp_days += 6;
        // else
        //     temp_days += s_month;
        temp_days += 6 *( temp_days/6);
        if (temp_days > 29){
            month++;
            temp_days -= 30;
        }
        ...
        // cout << s_month << endl;
        // s_day = days % 31 + ((days / 31) % 6) % 30;
        cout << s_year << "/" << s_month << "/" << temp_days;
        // I'm tired here is a quote: The reward of a thing well done is to have done it.
    }
